...
export const useEwcmController = (): EwcmController => {
  ...
  const [parameters, setParameters] = useState<Record<string, number>>(() => {
    ...
    if (stored) {
      // Validar corrección de datos antiguos (ej: limpieza de valor erróneo de Setpoint LP)
      if (stored['143-SEt'] !== undefined && stored['143-SEt'] < -0.9) {
        stored['143-SEt'] = 2.0; // Resetear a valor seguro si es una presión inválida
      }
  ...
  const executeAction = useCallback((actionId: string, index?: number) => {
    ...
    switch (actionId) {
      ...
      case 'reset_alarms':
        ...
        break;
      case 'silence_alarms':
        // TODO: Conectar con sistema de alarmas real cuando exponga el método silence
        console.log('Silenciando alarmas...');
        break;
      case 'factory_reset':
    ...
    switch (key) {
      ...
      case 'RIGHT':
        if (display.currentMenuId === 'root_home') {
          ...
          setParameters(prev => {
            const current = prev['548-UMFn'] || 0;
            return {
              ...prev,
              '548-UMFn': current === 0 ? 1 : 0
            };
          });
        } else if (display.passwordMode) {
      ...
      case 'ESC': // Tecla Izquierda/Atrás
        if (display.currentMenuId === 'root_home') {
          ...
          setParameters(prev => {
            const current = prev['547-UMCP'] || 0;
            return {
              ...prev,
              '547-UMCP': current === 0 ? 1 : 0
            };
          });
        } else if (display.passwordMode) {
      ...
      case 'F1': // Toggle ON/OFF del sistema
        actionCallbacksRef.current.onToggleSystem?.();
        break;
      case 'F2': // Acceso rápido a Setpoints
        if (display.currentMenuId === 'root_home') {
          // Navegar directamente a Menú Set/Banda
          setDisplay(prev => ({
            ...prev,
