...
const LcdScreen: React.FC<LcdScreenProps> = ({
  ...
  dateTimeTemp,
  dateTimeFieldIndex,
  quickStartEnabled = true, // Default true
  quickStartManual = false, // Default false
}) => {
  ...
  if (currentMenuId === 'root_home') {
    ...
    const suctionSet = parameters['143-SEt'] || -35.0;
  ...
  // --- RENDER: QUICK START ROOT (Pág 21) ---
  if (menuNode?.type === 'custom_quick_start') {
    return (
      <div className="w-full h-full bg-[#c8d4e0] text-black font-mono flex flex-col">
        {/* LINEA 1: Header */}
        <div className="bg-black text-white px-1 py-1 flex justify-between items-center text-xs h-7 shrink-0">
          <span className="uppercase font-bold tracking-wide text-[10px]">ENCENDIDO RAPIDO</span>
          <span className="font-mono bg-white text-black px-1 rounded-[1px] text-[10px]">01/01</span>
        </div>

        <div className="flex-1 p-1 flex flex-col gap-1">
          {/* Item 1: Activar */}
          <div className={`px-2 py-1 text-xs flex justify-between items-center border-b border-black/5 ${cursorIndex === 0 ? 'bg-black text-white' : ''}`}>
            <div className="flex items-center gap-2">
              {cursorIndex === 0 && <span className="text-[10px]">▶</span>}
              <span>Activar</span>
            </div>
            <span className="font-bold">{quickStartEnabled ? 'Si' : 'No'}</span>
          </div>

          {/* Item 2: Parámetros */}
          <div className={`px-2 py-1 text-xs flex justify-between items-center border-b border-black/5 ${cursorIndex === 1 ? 'bg-black text-white' : ''}`}>
            <div className="flex items-center gap-2">
              {cursorIndex === 1 && <span className="text-[10px]">▶</span>}
          ...
          {/* Item 3: Manual */}
          <div className={`px-2 py-1 text-xs flex justify-between items-center border-b border-black/5 ${cursorIndex === 2 ? 'bg-black text-white' : ''}`}>
            <div className="flex items-center gap-2">
              {cursorIndex === 2 && <span className="text-[10px]">▶</span>}
              <span>Manual</span>
            </div>
            <span className="font-bold">{quickStartManual ? 'Si' : 'No'}</span>
          </div>
        </div>
      </div>
    );
  ...
  // --- RENDER: WIZARD DE PARÁMETROS (Quick Start Params & Config) ---
  if (menuNode?.wizard) {
    // Modo Wizard: 1 parámetro por pantalla
    const currentItem = menuItems[cursorIndex];

    if (!currentItem) return <div className="w-full h-full bg-[#c8d4e0]" />;

    let valueDisplay = '...';
    let paramDescription = '';
    let paramIdDisplay = '';

    if (currentItem.parameterId) {
      const dbValue = (editMode && tempValue !== null) ? tempValue : parameters[currentItem.parameterId];
      const paramData = getParamData(currentItem.parameterId);

      // Lógica de Unidad de Visualización (Override)
      let displayValue = dbValue;
      let unitLabel = paramData ? getUnitLabel(paramData.unitType as any) : '';

      if (currentItem.displayUnit === 'PSI') {
        // Conversión Bar -> PSI (1 Bar = 14.5038 PSI)
        displayValue = dbValue * 14.5038;
        unitLabel = 'PSI';
      } else if (currentItem.displayUnit === 'F') {
        // Conversión C -> F ((C * 9/5) + 32)
        displayValue = (dbValue * 9 / 5) + 32;
        unitLabel = 'F';
      }

      paramDescription = paramData?.description || '';
      paramIdDisplay = currentItem.id;

      if (paramData?.type === 'enum' && paramData.options) {
        const option = paramData.options.find(o => o.value === Number(dbValue));
        valueDisplay = option ? option.label : String(dbValue);
      } else {
        const valNum = Number(displayValue);
        valueDisplay = valNum % 1 !== 0 ? valNum.toFixed(1) : String(valNum);
        valueDisplay += ` ${unitLabel}`;
      }
    }

    // Título id-dependiente
    const wizardTitle = menuNode.id === 'folder_config' ? 'CONFIGURACIÓN' : 'ENC.RAPIDO';

    return (
      <div className="w-full h-full bg-[#c8d4e0] text-black font-mono flex flex-col">
        {/* LINEA 1: Header */}
        <div className="bg-black text-white px-1 py-1 flex justify-between items-center text-xs h-7 shrink-0">
          <span className="uppercase font-bold tracking-wide text-[10px]">{wizardTitle}</span>
          <span className="font-mono bg-white text-black px-1 rounded-[1px] text-[10px]">
            {String(cursorIndex + 1).padStart(3, '0')}/{String(menuItems.length).padStart(3, '0')}
          </span>
        </div>

        {/* LINEA 2: ID - Code (e.g. 501 - tyPE) */}
        <div className="px-2 py-1 border-b border-black/10 flex items-center justify-center h-8">
          <span className="font-bold text-lg tracking-wider">
            {paramIdDisplay.replace('-', ' - ')}
          </span>
        </div>

        {/* LINEA 3: Descripción */}
        <div className="px-2 py-1 h-8 bg-black/5 text-[9px] leading-tight overflow-hidden text-black/80 italic flex items-center">
          {paramDescription}
        </div>

        {/* LINEA 4: Valor Editable */}
        <div className="flex-1 flex items-center justify-center bg-white/50 border-t border-black/10">
          <span className={`text-2xl font-bold font-mono ${editMode ? 'bg-black text-white px-2 animate-pulse' : 'text-black'}`}>
            {valueDisplay}
          </span>
        </div>
        {/* Footer Hint */}
        <div className="h-5 bg-black/10 flex items-center justify-center text-[8px] text-black/60">
          {editMode ? 'OK: Guardar | ESC: Cancelar' : 'OK: Editar | ▲▼: Navegar | ESC: Volver'}
        </div>
      </div>
    );
  }
